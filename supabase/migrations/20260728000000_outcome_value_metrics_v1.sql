-- Outcome Value Metrics v1.
--
-- Publishes one row per tenant without exposing clinical records. The metric
-- counts distinct source inferences, excludes synthetic provenance, and only
-- treats clinician-reviewed or laboratory-confirmed labels as confirmed.

create index if not exists idx_ai_inference_events_outcome_value
    on public.ai_inference_events (tenant_id, is_synthetic, id);

create index if not exists idx_clinical_outcome_events_outcome_value
    on public.clinical_outcome_events (tenant_id, inference_event_id, label_type)
...
comment on view public.outcome_value_metrics_v1 is
    'Tenant-level, privacy-preserving outcome value metrics. Distinct inferences only; synthetic provenance and inferred-only labels are excluded from confirmation counts.';
