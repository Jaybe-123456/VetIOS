...
const OUTCOME_CONTRIBUTION_URL = 'https://api.vetios.tech/v1/outcomes/contribute';

export async function dispatchOutcomeContribution(event: OutboxEvent): Promise<DeliveryResult> {
    const startedAt = Date.now();

    try {
        const response = await fetch(OUTCOME_CONTRIBUTION_URL, {
            method: 'POST',
            headers: {
        ...
        return {
            success: false,
            statusCode: response.status,
            error: readProviderError(responseBody, `Outcome contribution failed with status ${response.status}.`),
            durationMs,
            responseBody,
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Outcome contribution request failed.',
