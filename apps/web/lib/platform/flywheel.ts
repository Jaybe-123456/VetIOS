...
const ORPHAN_MONITOR_BATCH_SIZE = 250;
const DRIFT_MONITOR_INTERVAL_MS = 60 * 60 * 1000;

export async function runInferenceFlywheel(client: SupabaseClient, input: InferenceFlywheelInput) {
...
export async function ensureEvaluationForOutcome(
  ...
  if (outcomeError || !outcome) {
    throw new Error(
      `Outcome record not found for evaluation: ${outcomeError?.message ?? 'Unknown error'}`,
    );
  }

  const recentEvaluations = await getRecentEvaluations(client, input.tenantId, input.modelName, 20);
  const resolvedInferenceEventId =
    readText(input.inferenceEventId) ??
...
export async function listDatasetSnapshots(client: SupabaseClient, tenantId: string) {
  const { data, error } = await client
    .from('dataset_snapshots')
    .select('*')
...
export async function getOrphanCounter(client: SupabaseClient, tenantId: string) {
  const { data, error } = await client
...
export async function incrementOrphanCounter(client: SupabaseClient, tenantId: string) {
  const current = await getOrphanCounter(client, tenantId);
  const nextCount = current + 1;
  const { error } = await client.from('orphan_event_counters').upsert(
    {
      tenant_id: tenantId,
      count: nextCount,
...
async function getLatestDatasetSnapshot(client: SupabaseClient, tenantId: string) {
  const { data, error } = await client
    .from('dataset_snapshots')
    .select('*')
    .eq('tenant_id', tenantId)
...
export function startPlatformBackgroundJobs(client: SupabaseClient) {
  if (!shouldEnableRequestScopedPlatformJobs()) {
    return;
  }
...
function shouldEnableRequestScopedPlatformJobs() {
  if (process.env.VETIOS_ENABLE_PLATFORM_BACKGROUND_JOBS === 'true') {
    return true;
  }

  if (process.env.VETIOS_DISABLE_PLATFORM_BACKGROUND_JOBS === 'true') {
    return false;
  }

  // Request-scoped timers do not behave predictably on serverless runtimes,
  // so production maintenance work should run through explicit cron routes.
  return process.env.NODE_ENV !== 'production';
}

export async function runOrphanMonitor(client: SupabaseClient): Promise<OrphanMonitorSummary> {
  const threshold = new Date(Date.now() - ORPHAN_AGE_MS).toISOString();
  const windowStart = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

  // Fetch a small, oldest-first batch so the scan stays within serverless time budgets.
  const { data: candidates, error: inferenceError } = await client
    ...
    .limit(ORPHAN_MONITOR_BATCH_SIZE);

  if (inferenceError) {
    throw new Error(`Failed to scan for orphaned inference events: ${inferenceError.message}`);
  }

  const data = candidates ?? [];
  const inferenceIds = data.map((row) => String(row.id));

  // 2. Conditionally fetch ONLY the outcomes that map directly back to our active candidates
  let outcomes: Record<string, unknown>[] = [];
  if (inferenceIds.length > 0) {
    const { data: fetchedOutcomes, error: outcomesError } = await client
      .from('outcomes')
      .select('inference_event_id')
      .in('inference_event_id', inferenceIds);

    if (outcomesError) {
      throw new Error(
        `Failed to load linked outcomes during orphan scan: ${outcomesError.message}`,
      );
    }
    outcomes = (fetchedOutcomes ?? []) as Record<string, unknown>[];
  }
  ...
  for (const row of (data ?? []) as Array<Record<string, unknown>>) {
    ...
    const backfilled = await backfillInferenceEvaluation(client, {
      actor: {
        userId: 'system',
        tenantId,
        role: 'system_admin',
    ...
    backfilledEvaluations += backfilled.length;
  }

  await emitOrphanRateAlerts(client);

  return {
    scanned_candidates: data.length,
    linked_candidates: linkedInferenceIds.size,
    orphaned_candidates: orphanedCandidates,
    backfilled_evaluations: backfilledEvaluations,
  };
}
...
export async function backfillInferenceEvaluation(
  ...
  const filteredQuery = input.inferenceEventId ? query.eq('id', input.inferenceEventId) : query;
...
export async function runDriftDetection(client: SupabaseClient) {
  ...
  if (tenantsError) {
    throw new Error(
      `Failed to load evaluation tenants for drift detection: ${tenantsError.message}`,
    );
  }
  ...
  for (const { tenantId, modelVersion } of uniquePairs.values()) {
    const currentScores = await listEvaluationScores(
      client,
      tenantId,
      modelVersion,
      currentStart,
    ...
    const baselineScores = await listEvaluationScores(
      client,
      tenantId,
      modelVersion,
      baselineStart,
...
async function upsertDriftSnapshot(client: SupabaseClient, result: DriftDetectionResult) {
  const { error } = await client.from('drift_snapshots').upsert(
    {
...
function standardDeviation(values: number[], providedMean: number | null) {
  ...
  const variance =
    values.reduce((sum, value) => sum + (value - providedMean) ** 2, 0) / values.length;
