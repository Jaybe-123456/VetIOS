...
export async function GET(
    ...
    try {
        ...
        return NextResponse.json({
            ...
            error: {
                code: typeof (error as { code?: string })?.code === 'string' ? (error as { code: string }).code : 'sovereign_run_load_failed',
                message: error instanceof Error ? error.message : 'Failed to load Sovereign run.',
            },
        }, { status });
    }
}
