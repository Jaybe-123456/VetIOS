...
export default function PassiveSignalsPage() {
    return (
        <PlatformShell
            ...
            description="VetIOS already normalizes passive connector events into episode-aware signals. This surface publishes the supported connector types, the normalized facts each one contributes, and where the engine still needs broader vendor sync coverage."
            actions={(
                <>
                    <Link
                        href="/outcome"
        ...
        >
            <div className="grid gap-4 md:grid-cols-3">
                ...
                <SummaryCard label="Planned connectors" value={String(passiveSignalConnectors.filter((connector) => connector.readiness === 'planned').length)} />
                <SummaryCard label="Current ingest path" value="/api/signals/connect" />
            </div>
