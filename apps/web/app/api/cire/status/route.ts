...
import { getCireStatus } from '@/lib/cire/engine';
...
export async function GET(req: Request) {
    ...
    try {
        ...
        const status = await getCireStatus(supabase, resolvedTenantId);
        ...
        const response = error instanceof PlatformRateLimitError
            ...
            : NextResponse.json({
                ...
                error: {
                    code: error instanceof PlatformAuthError ? error.code : 'cire_status_failed',
                    message: error instanceof Error ? error.message : 'Failed to load CIRE status.',
                },
