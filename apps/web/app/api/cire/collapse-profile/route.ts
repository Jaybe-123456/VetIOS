...
import { getLatestCollapseProfile } from '@/lib/cire/engine';
...
export async function GET(req: Request) {
    ...
    try {
        ...
        const profile = await getLatestCollapseProfile(
            supabase,
            resolvedTenantId,
            url.searchParams.get('model_version'),
        );
        ...
        const response = error instanceof PlatformRateLimitError
            ...
            : NextResponse.json({
                ...
                error: {
                    code: error instanceof PlatformAuthError ? error.code : 'cire_profile_failed',
                    message: error instanceof Error ? error.message : 'Failed to load CIRE collapse profile.',
                },
