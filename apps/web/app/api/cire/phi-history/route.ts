...
import { getPhiHistory } from '@/lib/cire/engine';
...
export async function GET(req: Request) {
    ...
    try {
        ...
        const history = await getPhiHistory(supabase, {
            tenantId: resolvedTenantId,
            from: url.searchParams.get('from'),
            to: url.searchParams.get('to'),
            granularity: (
                url.searchParams.get('granularity') === 'minute'
                || url.searchParams.get('granularity') === 'hour'
                || url.searchParams.get('granularity') === 'day'
            )
                ? url.searchParams.get('granularity') as 'minute' | 'hour' | 'day'
                : 'hour',
        });
        ...
        const response = error instanceof PlatformRateLimitError
            ...
            : NextResponse.json({
                ...
                error: {
                    code: error instanceof PlatformAuthError ? error.code : 'cire_phi_history_failed',
                    message: error instanceof Error ? error.message : 'Failed to load CIRE history.',
                },
