...
import { getLatestEvaluationEventId } from '@/lib/debugTools/service';
...
export async function GET(req: Request) {
    ...
    try {
        const eventId = await getLatestEvaluationEventId(access.access.client, access.access.tenantId);
        ...
        const response = NextResponse.json(
            {
                error: error instanceof Error ? error.message : 'Failed to load latest evaluation event.',
                request_id: guard.requestId,
            },
            { status: 500 },
        );
