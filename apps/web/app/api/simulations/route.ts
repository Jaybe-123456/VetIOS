...
import { listSimulationRuns } from '@/lib/platform/simulations';
...
export async function GET(req: Request) {
    ...
    try {
        ...
        const list = await listSimulationRuns(supabase, {
            tenantId: resolvedTenantId,
            mode: mode === 'load' || mode === 'adversarial' || mode === 'regression' ? mode : null,
            status: status === 'pending' || status === 'running' || status === 'complete' || status === 'failed' || status === 'blocked' ? status : null,
            cursor,
            limit,
        });
        ...
        const response = error instanceof PlatformRateLimitError
            ...
            : NextResponse.json({
                ...
                error: {
                    code: error instanceof PlatformAuthError ? error.code : 'simulations_list_failed',
                    message: error instanceof Error ? error.message : 'Failed to list simulations.',
                },
